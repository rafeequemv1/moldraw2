...
export interface MoleculeSelection {
  ...
  /** Marquee / multi-select: all annotation ids currently in the selection set. */
...
export interface MoleculeEditor {
  ...
  /**
   * Replace the present molecule without touching undo history (live scrubbing).
   * Pair with `commitUndoFromBaseline` when the gesture ends.
   */
  replacePresentWithoutHistory(updater: Molecule | ((prev: Molecule) => Molecule)): void;
  /**
   * Replace the present molecule and clear undo/redo (project load / new document).
   */
  resetMolecule(molecule?: Molecule): void;
  /**
   * After a without-history scrub, push `baseline` onto the undo stack so Undo
   * restores the pre-scrub molecule. No-op when present equals baseline.
   */
  commitUndoFromBaseline(baseline: Molecule): void;
  undo(): void;
  redo(): void;
  canUndo(): boolean;
  canRedo(): boolean;
