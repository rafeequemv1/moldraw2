...
import type { CipStereoTags } from '@moldraw/engine-2d';
import type { StructureCheckResult } from '@moldraw/engine-2d';
import type { IndigoConvertFormat } from '@moldraw/engine-2d';
import type { IndigoCalculatedProperties } from '@moldraw/engine-2d';
import type { IndigoDruglikeProperties } from '@moldraw/engine-2d';
import type { AutomapMode } from '@moldraw/engine-2d';
...
export type MoleculeWorkerRequest =
  ...
  | {
      ...
      payload: {
        molBlock: string;
        /** fold = remove explicit H; unfold = add; auto = toggle. */
        mode?: 'fold' | 'unfold' | 'auto';
      };
    }
  | {
...
export type MoleculeWorkerResponse =
  ...
  | {
      ...
      id: string;
      payload: { molBlock: string; mode: 'fold' | 'unfold' | 'auto' };
    }
  | { type: 'CONVERT_EXPLICIT_HYDROGENS_ERROR'; id: string; error: string }
  | {
