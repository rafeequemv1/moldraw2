 ...
 *
 * Local bond/atom tweaks must use region refine (keep last good 3D, move only
 * the dirty neighborhood). Import / paste / Calculate structure always use the
 * same progressive center-out path for every molecule size (cholesterol,
 * paclitaxel, etc.) — not only large ones.
 */
