 ...
 */
export { embed3D, applySeed3D, type Conformer, type Atom3D, type Embed3DOptions } from './embed';
export {
  embedDistanceGeometry,
  heavyAtomStrain,
  isStrainedConformer,
  DG_MAX_ATOMS,
  type DistanceGeometryInput,
} from './distanceGeometry';
export { snapCoordinationPolyhedra } from './coordinationSeed';
export {
  coordinationGeometryFor,
  isUffMetal,
  type CoordinationGeometry,
} from './forcefield/uffParams';
...
export { structureKeyFor3D } from './structureKey3D';
export { pickFocusFragment, retainPinnedFocus, type FocusFragmentResult } from './focusFragment';
export {
  enforceAndVerifyStereo3D,
  enforceStereoConstraints,
  enforceStereoOnMolblock3D,
  verifyStereoConstraints,
  moleculeHasStereoConstraints,
  type StereoStatusItem,
  type StereoVerifyResult,
  type ChiralStereoStatus,
  type CisTransStereoStatus,
} from './verifyStereo3D';
export { perceiveStereo, type StereoConstraints, type ChiralConstraint, type CisTransConstraint } from './stereo';
export { applyDepictionStereoSeed, bakeWedgeDashSeed } from './stereoSeed';
export { unthreadSeed } from './unthread';
...
export function generate3DMolblock(mol: Molecule, opts: Generate3DOptions = {}): string {
  ...
  if (sampleCount <= 1 || opts.forceField === 'none' || opts.seed3D) {
    // A 3D seed means "re-minimize this conformer" — torsional sampling would
    // throw the starting geometry away, so always take the single-embed path.
