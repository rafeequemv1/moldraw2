/**
 * Reaction library modal — categories, steps, conditions, mechanism; insert full scheme.
 */
import { useEffect, useMemo, useState } from 'react';
import { ArrowRight, ChevronRight, Folder, Star, X } from 'lucide-react';
import {
...
export function ReactionLibraryModal({
  open,
  onClose,
  onInsert,
  insertingId = null,
  ...
  useEffect(() => {
    if (!open || listReactions.length === 0) {
      setSelectedId(null);
      return;
    }
        ...
        <header className="template-library-header">
          <div>
            ...
            <div className="template-library-header__subtitle">
              Famous named reactions — insert full schemes with arrows, conditions, and labels.
            </div>
          </div>
          ...
          <aside className="template-library-sidebar">
            ...
            <nav className="template-library-nav" aria-label="Reaction categories">
              {filteredCategories.map(cat => {
                const active = !searchQuery && cat.id === categoryId;
