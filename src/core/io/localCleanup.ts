 ...
 * rotated and/or mirrored relative to the user's drawing. Without the
 * Procrustes step the structure visually flips even though semantics are
 * identical, which feels jarring. The alignment recovers the user's intended
 * orientation. Reflections are allowed only when the subset has no
 * wedge/dash/wavy bonds (mirroring inverts the depicted chirality even
 * though the stereo *property* is preserved).
 *
 ...
 *
 * Cleaned coordinates are scaled to the subset's average bond length and
 * centered on the subset's original centroid — the layout is not rigid-fitted
 * back onto the original atom positions (which would undo the cleanup).
 */
import type { Molecule, Point } from '@moldraw/domain';
import { parseMolblock } from './molblock';
...
export const spliceLocalCleanup = (
  ...
  // Layout is generated at the target bond length — translate only, no rescale.
  const aligned = mapCleanupPositions(cleanedPts, originals, 1);
 ...
 * atom/bond ids and presentation, adopt cleaned coordinates ONLY.
 *
...
export const mergeGlobalCleanup = (prev: Molecule, cleanedMolblock: string): Molecule => {
  ...
  const finish = (atoms: Molecule['atoms']): Molecule => ({
    ...prev,
    atoms,
  ...
  if (prev.atoms.length === cleaned.atoms.length) {
    const mapped = mapCleanupPositions(
      cleaned.atoms.map(a => ({ x: a.x, y: a.y })),
      prev.atoms.map(a => ({ x: a.x, y: a.y })),
      1,
    );
    if (!mapped) return prev;
    return finish(
      prev.atoms.map((prevAtom, i) => ({
        ...prevAtom,
        x: mapped[i].x,
        y: mapped[i].y,
      })),
  ...
  const mappedHeavy = mapCleanupPositions(
    heavyLaid.map(a => ({ x: a.x, y: a.y })),
    heavyPrev.map(a => ({ x: a.x, y: a.y })),
    1,
  );
  if (!mappedHeavy) return prev;

  const coordById = new Map<string, { x: number; y: number }>();
  for (let i = 0; i < heavyPrev.length; i++) {
    coordById.set(heavyPrev[i]!.id, mappedHeavy[i]!);
  }
  return finish(
    prev.atoms.map(a => {
