...
export function previewFragmentPlacement(
  ...
  if (snapTargetAtomId && connectionAtomId) {
    const placed = layoutFragmentForAttachment(
      parent,
      fragment,
      connectionAtomId,
      snapTargetAtomId,
  ...
  placed = nudgeFragmentClearOfParent(parent, placed, new Set(), minSep);
...
export function commitFragmentPlacement(
  ...
  placed = nudgeFragmentClearOfParent(prev, placed, new Set(), minSep);
  return {
    molecule: {
      ...prev,
