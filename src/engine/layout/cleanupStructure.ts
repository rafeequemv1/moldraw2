...
export const cleanupStructureWithStatus = (
  ...
  if (isChemDrawReady(next)) {
    ...
    return { molecule: mergeHeavyCoords(mol, next, bondLen), status: 'certified' };
  }
  ...
  if (isChemDrawReady(next) || passesSoftGate(next)) {
    ...
    if (isChemDrawReady(next) || passesSoftGate(next)) {
      return {
        molecule: mergeHeavyCoords(mol, next, bondLen),
  ...
  return { molecule: mergeHeavyCoords(mol, next, bondLen), status };
};
