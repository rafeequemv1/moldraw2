...
import { getAtomValency, pickAtomAt, pickAtomOrBondForRingTool } from './hitTest';
...
export const ringToolMouseDown = (ctx: InteractionContext): boolean => {
  ...
  const { atom, bond } = pickAtomOrBondForRingTool(molecule, worldPos);
  if (atom) {
...
export const ringToolMouseUp = (ctx: InteractionContext): boolean => {
  ...
  const releasePick = pickAtomOrBondForRingTool(molecule, worldPos);
